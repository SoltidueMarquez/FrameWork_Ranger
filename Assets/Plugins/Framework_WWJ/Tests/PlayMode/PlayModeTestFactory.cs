    ...
    internal sealed class PlayModeTestFactory
    {
            ...
            FrameworkSceneConfig sceneConfig)
        {
            ...
            ObserveOperationAsync(
                Framework.StartProjectSceneAsync(
                    m_projectSettings,
                    new FrameworkSceneDescriptor(handle, $"Tests/Scene_{handle}.unity", $"Scene_{handle}"))).Forget();
            return handle;
        }
        ...
        internal int ActivateScene(FrameworkProjectSettings settings, string scenePath)
        {
            ...
            ObserveOperationAsync(
                Framework.StartProjectSceneAsync(
                    settings,
                    new FrameworkSceneDescriptor(handle, scenePath, $"Scene_{handle}"))).Forget();
            return handle;
        }
        ...
        internal void ActivateWithoutProjectSettings()
        {
            ObserveOperationAsync(
                Framework.StartProjectSceneAsync(
                    null,
                    new FrameworkSceneDescriptor(m_nextSceneHandle++, "Tests/MissingSettings.unity", "MissingSettings"))).Forget();
        }
        ...
        {
            ObserveOperationAsync(Framework.DetachSceneAsync(sceneHandle)).Forget();
        }
