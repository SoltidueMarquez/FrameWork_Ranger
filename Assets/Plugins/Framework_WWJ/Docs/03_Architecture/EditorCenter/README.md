...
本目录记录 Phase 1.1 的中央启动、统一编辑器入口与声明式代码架构图设计，Phase 1.2 的视觉层级、显式页面发现边界和共享可导航图视口，Phase 1.3 的预览/固定页签交互，Phase 1.4 的生产程序集接入，Phase 1.5 的单画布可展开代码架构图，以及 Phase 1.6 的紧凑复合节点与会话拖动。具体运行时代码仍以 Core 契约为基础；本目录只增加项目级配置和编辑器工作流，不改变模块生命周期、依赖排序与回滚语义。
...
15. [中央启动 ADR](../Core/ADR/ADR-006_Central_Project_Settings_And_Scene_Ownership.md)
16. [Editor Center ADR](./ADR/README.md)

## 当前状态

- **状态**：Phase 1.1–1.6 已实现；Phase 1.6 自动化为 EditMode 73/73、PlayMode 17/17，真实窗口视觉交互等待用户确认。
