...
> 检查日期：2026-08-19  
> 默认权限：只读  
> 用途：补充 Framework_WWJ 在可选模块、所有权、资源适配与工具链方面的设计证据；不要求兼容或复制 YokiFrame API。

## 1. 信息标记

- **事实**：已从 YokiFrame 的源码、项目文档或项目 Skill 中确认。
- **推断**：根据多处实现归纳的意图，仍可继续验证。
- **候选**：值得 Framework_WWJ 讨论的方向，尚未成为决策。
...
这与 Framework_WWJ 的目标相似之处，是把能力拆成可按需组合的单元；不同之处是 Framework_WWJ 当前先以 Unity 内的 Module SO 和统一 Scope 生命周期为主，不直接采用 YokiFrame 的静态 Kit API。
...
## 4. 对 Framework_WWJ 的启示

| 主题 | 参考事实 | Framework_WWJ 候选方向 |
| --- | --- | --- |
...
- 不把静态 Kit 门面、命名、线程策略或引擎兼容目标直接搬进 Framework_WWJ。
