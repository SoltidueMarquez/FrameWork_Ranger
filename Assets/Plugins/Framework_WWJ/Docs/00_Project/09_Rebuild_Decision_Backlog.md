...
Unity 6000.5.9f1 与 `FrameWork_Ranger` 仓库迁移已由 [ADR-DIST-001](../03_Architecture/Distribution/ADR-DIST-001_Unity6_Repository_Migration.md) 接受；该决定只解决源工程版本、仓库历史、依赖基线、验证与旧仓库清理门禁。用户提出的未来专用 App 方向仍用于管理 Framework_WWJ 源码、选择模块、向游戏项目安装/更新，并支持在游戏开发期间继续维护框架代码。研究入口见[分发 App 探索](../03_Architecture/Distribution/00_Framework_Repository_Distribution_App_Exploration.md)。
