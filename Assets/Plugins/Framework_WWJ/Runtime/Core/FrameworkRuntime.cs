...
namespace Framework_WWJ
{
    ...
    internal sealed class FrameworkRuntime
    {
        ...
        internal UniTask DetachSceneAsync(int sceneHandle)
        {
        ...
        private async UniTask DetachSceneCoreAsync(int sceneHandle)
        {
            ...
            CancellationToken sceneCancellationToken)
        {
                       ...
                       m_shutdownCancellation.Token))
            {
                ...
                catch (Exception exception)
                {
                    ...
                    Debug.LogError($"[Framework_WWJ] GlobalScope 初始化失败：{exception.Message}" );
                    throw;
                }
            }
        }
            ...
            CancellationToken sceneCancellationToken)
        {
                       ...
                       m_shutdownCancellation.Token))
            {
                ...
                catch (Exception exception)
                {
                    ...
                    LastException = exception;
                    FailReadiness(exception);
                    Debug.LogError($"[Framework_WWJ] SceneScope 加载失败：{exception.Message}" );
                    throw;
                }
            }
        }
        ...
        private static void LogCleanupErrors(string phase, IReadOnlyList<Exception> errors)
        {
            ...
            {
                Debug.LogError($"[Framework_WWJ] {phase}时发生异常，清理流程继续执行。" );
