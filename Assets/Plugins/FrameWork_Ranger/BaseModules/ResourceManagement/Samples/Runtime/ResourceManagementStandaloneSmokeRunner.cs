...
namespace FrameWork_Ranger.ResourceManagement.Samples
{
    ...
    {
        private const string CommandLineFlag = "-frameworkRangerResourceSmoke";
        ...
        private static async UniTaskVoid RunAsync()
        {
            ...
            try
            {
                ...
                Debug.Log("[FrameWork_Ranger][ResourceStandaloneSmoke] PASS 双后端 Acquire/Instantiate/Destroy/Release。");
            }
            catch (Exception exception)
            {
                exitCode = 1;
                Debug.LogError($"[FrameWork_Ranger][ResourceStandaloneSmoke] FAIL {exception}");
            }
            finally
            {
                ...
                catch (Exception shutdownException)
                {
                    exitCode = 1;
                    Debug.LogError(
                        $"[FrameWork_Ranger][ResourceStandaloneSmoke] Shutdown FAIL {shutdownException}");
                }
