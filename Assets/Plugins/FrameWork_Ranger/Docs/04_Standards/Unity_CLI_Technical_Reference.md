# FrameWork_Ranger Unity CLI 技术参考
...
本文解释 FrameWork_Ranger 的 Unity CLI 包装层如何定位 Editor、避免工程争用、等待 Windows GUI 进程、组织测试与构建、判定失败以及扩展新任务。强制约束以 [Unity CLI 开发规则](./Unity_CLI_Development_Rules.md) 为准；只想运行命令时阅读 [Unity 6000 CLI 开发与验证](./Unity_6000_CLI.md)。
...
| 默认 Player | `Builds/UnityCli/FrameWork_Ranger.exe` | `Tools/UnityCli.ps1` |
...
| `-PlayerPath` | Player 输入或输出路径 | `Builds/UnityCli/FrameWork_Ranger.exe` |
...
精确比较避免把 `FrameWork_Ranger` 与 `FrameWork_Ranger_Copy` 这类前缀相同的项目误判为同一工程。Asset Import Worker 也携带 `-projectPath`，因此主 Editor 或其活跃 Worker 都能触发占用保护。
...
脚本先验证 Player 存在，再等待 Player 退出并检查退出码。`-frameworkRangerResourceSmoke` 的业务断言由 Resource Management Player 入口负责，必须覆盖 Resources 与 Addressables 两个后端。
