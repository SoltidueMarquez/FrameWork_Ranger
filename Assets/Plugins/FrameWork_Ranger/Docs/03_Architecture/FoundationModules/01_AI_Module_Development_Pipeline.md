...
> 自动发现 Skill：`$build-framework-ranger-module`。  
> 前置 Skill：`$work-with-framework-ranger`；涉及核心契约时同时使用 `$framework-ranger-lightweight-refactor`。
...
- 始终使用统一的 `$build-framework-ranger-module` 流水线 Skill。
