...
本目录是 FrameWork_Ranger 资源管理模块的权威事实源。Runtime 实现位于 `BaseModules/ResourceManagement`，模块采用显式后端键、独立 Lease、同键同类型 single-flight，并同时提供 Unity Resources 与 Addressables 1.22.3。
