...
> 以下保留第一阶段骨架计划的历史交付规格。2026-09-05 起新任务按下述现行用法执行，历史的全字段/全方法冻结与全套测试条款不自动作为新阶段门禁。

## 现行使用方法

先明确用户目标、公共行为、所有权、重要数据流和目标路径。关键脚本说明职责和协作，复杂关系用图；内部字段和辅助方法允许在授权范围内调整，用户明确要求完整逐脚本设计时再提供相应细节。

需求、问答、决定与进度按[任务记忆协议](../../05_Skills/01_Task_Memory_And_Recovery.md)保存。已有实现授权不重复索取；公共契约的新选择只确认受影响部分。验证按[Unity CLI 规则](../../04_Standards/Unity_CLI_Development_Rules.md)选择，与本次改动无关的历史验收场景不自动重跑。

后续路径使用当前工程的 `Assets/Plugins/FrameWork_Ranger`，历史文字中的旧产品名和 Entry 路径仅用于理解当时方案。
