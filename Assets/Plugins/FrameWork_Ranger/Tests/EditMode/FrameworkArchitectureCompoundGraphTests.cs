...
namespace FrameWork_Ranger.Tests
{
    internal sealed class FrameworkArchitectureCompoundGraphTests
    {
        ...
        public void SetUp()
        {
            m_expansionSessionKey = $"FrameWork_Ranger.Tests.ArchitectureExpansion.{Guid.NewGuid():N}";
            m_positionSessionKey = $"FrameWork_Ranger.Tests.ArchitecturePosition.{Guid.NewGuid():N}";
        ...
        public void Search_TemporarilyExpandsMatchPathWithoutChangingUserOrPositionState()
        {
            var resourceModule = m_catalog.Nodes.Single(node =>
                node.Type.FullName == "FrameWork_Ranger.ResourceManagement.ResourceModule");
